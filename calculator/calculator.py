...
allowed_functions = {
    ...
    "sqrt":math.sqrt,
    "e":math.e,
    'rad': math.radians,  # Optional: convert degrees to radians
...
def create_circle_button(parent, text, row, col):
    ...
    # Tag both with the button text so we can identify them later
    canvas.addtag_withtag(text, circle)
    canvas.addtag_withtag(text, label)

    # Bind your click function to both the circle and label
